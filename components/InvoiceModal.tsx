...
import { Invoice, InvoiceStatus, InvoiceInput } from '@/lib/types'
import { createInvoice, updateInvoice } from '@/lib/dataSource'
...
function toItemDraft(item: Invoice['items'][number]): ItemDraft {
  return {
    key: item.id,
    name: item.name,
...
function resolveInitialItems(invoice?: Invoice, fromEstimate?: FromEstimateData): ItemDraft[] {
  if (invoice && invoice.items.length > 0) return invoice.items.map(toItemDraft)
  if (fromEstimate && fromEstimate.items.length > 0) {
    return fromEstimate.items.map((item) => ({
...
export default function InvoiceModal({ projectId, customerId, invoice, fromEstimate, taxRate, invoiceDueDays, onClose, onSaved }: Props) {
  ...
  const [title, setTitle] = useState(() => {
    ...
    if (fromEstimate) return fromEstimate.title
    return ''
  })
  ...
  const [note, setNote] = useState(() => {
    if (invoice) return invoice.note ?? ''
    if (fromEstimate) return fromEstimate.note ?? ''
    return ''
  })
  const [items, setItems] = useState<ItemDraft[]>(() => resolveInitialItems(invoice, fromEstimate))
      ...
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-xl">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 shrink-0">
          <div>
            <h2 className="text-sm font-semibold text-gray-900">
              {isEdit ? '請求書を編集' : fromEstimate ? '見積書から請求書を作成' : '請求書を作成'}
            </h2>
            {fromEstimate && !isEdit && (
              <p className="text-xs text-gray-400 mt-0.5">見積書の明細が引き継がれています</p>
            )}
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
            ...
            <div className="grid grid-cols-[1fr_140px] gap-3">
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1.5">
                  請求タイトル <span className="text-red-400">*</span>
                </label>
                <input
                  type="text"
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
                  placeholder="例：Webサイトリニューアル 御請求書"
            ...
            {/* 支払期限 */}
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1.5">支払期限</label>
              <input
                type="date"
                value={dueDate}
                    ...
                    <div key={item.key} className="bg-gray-50 rounded-xl p-3 space-y-2">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="flex items-center gap-0.5 shrink-0">
                          ...
                          <button type="button" onClick={() => removeItem(item.key)} disabled={items.length === 1}
                      ...
                      <div className="flex items-center gap-2 ml-6 flex-wrap">
                        <div className="flex items-center gap-1.5">
                          <input
                            ...
                            min="0.01" step="0.01" placeholder="数量"
